...
export const Route = createFileRoute("/")({
  head: () =>
    pageSeo({
      title:
        "TTS Trustable Technical Services | Scaffolding Rental & Construction Equipment Rwanda",
      description:
        "Established in 2014, Trustable Technical Services Ltd provides cuplock scaffolding rental, construction equipment and maintenance services for major projects in Kigali and across Rwanda.",
      path: "/",
    }),
  component: Index,
});
...
function Counter({ end, suffix = "" }: { end: number; suffix?: string }) {
  ...
  return (
    <div ref={ref} className="text-4xl font-serif text-white">
  ...
  return (
    <span className="inline-grid">
      {words.map((word, i) => (
        <span
          key={word}
          className={`col-start-1 row-start-1 transition-all duration-1000 ${
    ...
    <SiteLayout transparentHeader>
        ...
        <div className="relative flex h-full flex-col justify-end items-center pb-32 md:pb-28">
          <div className="mx-auto w-full max-w-7xl px-6 lg:px-10 flex flex-col items-center text-center">
            <div className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-background/70">
              <span className="h-px w-8 bg-primary-glow" /> Est. 2014 · Kigali{" "}
            ...
            <h1 className=" mt-5 max-w-3xl font-serif text-3xl leading-snug text-background text-balance md:text-4xl lg:text-5xl">
              <span className="text-h1-hero">Trustable Technical Services</span>
              <br />
              <span className="italic text-primary-glow text-hero">
                for Rwanda's Flagship Projects.
              </span>
            </h1>
            <div className="mt-3 flex flex-wrap justify-center gap-3">
              <Link
                to="/contact"
                className="rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground shadow-[var(--shadow-elev)] transition-all hover:bg-primary-glow hover:text-ink"
              >
                Start a project
              </Link>
              <Link
                to="/projects"
                className="rounded-full border border-background/40 px-6 py-3 text-sm font-medium text-background backdrop-blur-md transition-all hover:bg-background/10"
              >
                See our work →
              </Link>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6 pt-16 pb-28 lg:px-10">
          ...
          <FadeIn>
            <div className="flex flex-col items-center justify-center gap-6 text-center">
              ...
              <Link
                to="/services"
                className="text-sm font-medium text-primary hover:text-primary-deep md:inline"
              >
                All services →
              </Link>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <FadeIn>
            <div className="flex flex-wrap items-end justify-between gap-6">
              <div>
                <div className="text-xs uppercase tracking-[0.22em] text-primary-glow">
                  Selected work
                </div>
              ...
              <Link
                to="/projects"
                className="text-sm font-medium text-primary-glow hover:text-background"
              >
                View portfolio →
              </Link>
            </div>
            ...
            {[
              {
                ...
                tag: "Ongoing · Scaffolding",
              },
              {
                img: kccImg,
                name: "Kigali Convention Centre",
                tag: "Renovation · Access scaffolding",
              },
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-15 lg:grid-cols-2 lg:px-10">
        <FadeIn className="flex flex-col justify-center">
          ...
          <h2 className="mt-4 font-serif text-4xl leading-tight font-extrabold text-ink md:text-5xl">
            A decade of <em className="text-primary ">trustable</em> work.
          </h2>
          <div className=" mt-6 space-y-6 text-lg leading-relaxed text-foreground/80 lg:col-span-7">
            <p>
              Established in 2014, Trustable Technical Services (TTS) Ltd is a diversified
              investment company specialised in construction-related fields — scaffolding, machinery
              and equipment, and maintenance services.
            </p>
            <p>
              We direct our work into high-impact projects, creating employment for skilled youth
              while delivering uncompromising quality to our partners and attractive returns to our
              shareholders.
            </p>
          </div>
          <Link
            to="/about"
            className="mt-6 inline-flex items-center gap-2 pt-2 text-sm font-medium text-primary hover:text-primary-deep"
          >
        ...
        <FadeIn delay={200} className="relative overflow-hidden rounded-3xl bg-ink">
          <img
            src={heroPoster}
            alt="Trustable Technical Services scaffolding"
            loading="lazy"
            width={1200}
            height={900}
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-10 lg:grid-cols-2 lg:px-10">
        <FadeIn className="relative overflow-hidden rounded-3xl">
          <img
            src={productCuplock}
            alt="Cuplock scaffolding components"
            loading="lazy"
            width={1200}
            height={900}
            className="h-full w-full object-cover"
        ...
        <FadeIn delay={200} className="flex flex-col justify-center">
          ...
          <h2 className="mt-4 font-serif text-4xl text-ink md:text-5xl font-bold">TTS Equipment</h2>
          <p className="mt-6 text-lg leading-relaxed text-foreground/75">
            Our cuplock system is engineered for speed, safety and repeatable geometry — the same
            kit that wraps Rwanda's national stadiums and convention centres during renovation
            works.
          </p>
          <ul className="mt-8 space-y-3 text-sm text-foreground/75">
            {[
              "Hot-dip galvanized for corrosion resistance",
              "Compatible ledgers, standards & braces",
              "Trained crews available on request",
            ].map((x) => (
          ...
          <Link
            to="/services"
            className="mt-10 inline-flex w-fit items-center gap-2 rounded-full border border-ink/15 px-6 py-3 text-sm font-medium text-ink transition-all hover:bg-ink hover:text-background"
          >
          ...
          <div className="relative grid gap-10 lg:grid-cols-2 lg:items-center">
            ...
            <div className="lg:text-right">
              <p className="max-w-md text-foreground/70 lg:ml-auto">
                Tell us your scope, timeline and site. We'll respond with the right equipment, crew
                and a clear plan.
              </p>
              <Link
                to="/contact"
                className="mt-8 inline-flex items-center gap-2 rounded-full bg-ink px-7 py-3.5 text-sm font-medium text-background transition-all hover:bg-primary"
              >
